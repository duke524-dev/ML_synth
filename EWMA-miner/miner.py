...
class EWMAMiner(BaseMinerNeuron):
    """
    EWMA-based miner using stateful volatility models with Student-t innovations
    """
    ...
    def __init__(self, config=None):
        ...
        self.forecast_engine = ForecastEngine(state_dir=state_dir)
    ...
    async def forward_miner(self, synapse: Simulation) -> Simulation:
        ...
        bt.logging.info(
            f"[EWMAMiner] Request from {synapse.dendrite.hotkey} "
        ...
        try:
            ...
            bt.logging.info(
                f"[EWMAMiner] Generated {simulation_input.num_simulations} paths "
                f"for {simulation_input.asset}"
            )
            
        except Exception as e:
            bt.logging.error(f"[EWMAMiner] Error in forward_miner: {e}", exc_info=True)
